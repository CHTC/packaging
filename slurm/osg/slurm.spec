...
# Enable pmix in support of INF-1874
%define with_pmix 1
...
BuildRequires: openpmix
BuildRequires: openpmix-devel
%global pmix_version %(rpm -q openpmix --qf "%{RPMTAG_VERSION}")
%endif
...
Requires: %{name}%{?_isa} = %{version}-%{release}
%if %{with pmix} && "%{_with_pmix}" == "--with-pmix"
Requires: openpmix = %{pmix_version}
Requires: openpmix-lib = %{pmix_version}
%endif
%if %{with ucx} && "%{_with_ucx}" == "--with-ucx"
